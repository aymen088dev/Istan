...
export function SquadManager({ club, onChange, onBestXI, onBestXIAI, onRecommendFormation, formation, sport = "football" }: Props) {
  ...
  const activeAnalysis = analyzeFormation(activeFormation, formationMap[activeFormation], roster, sport as AnalysisSport);
  const recommendedFormation = analyzeAllFormations(formationMap, roster, sport as AnalysisSport)[0];
